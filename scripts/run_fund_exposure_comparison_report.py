...
from src.scanners.fund_exposure_comparison_report import (  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run a Can-Do multi-fund exposure comparison report."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        json.dumps(
            {
                "json": str(output_dir / "fund_exposure_comparison_report.json"),
                "html": str(output_dir / "fund_exposure_comparison_report.html"),
...
def _write_outputs(*, output_dir: Path, report: dict[str, Any]) -> None:
    (output_dir / "fund_exposure_comparison_report.json").write_text(
        json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    (output_dir / "fund_exposure_comparison_report.html").write_text(
