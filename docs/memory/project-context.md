...
- Announcement provider payloads and generated announcement evidence payloads have reusable contract validators. Optional CNINFO orchestration validates the provider payload before conversion and the generated evidence payload before returning it.
- Optional market quote, valuation snapshot, financial metrics, and news evidence payloads are also validated at the orchestration boundary, so injected providers and future adapters cannot bypass provider contracts.
...
- Reviewed-mapping enriched acceptance also validates workspace snapshot `data_layers`: holdings, announcements, announcement evidence, market quotes, valuation snapshots, financial metrics, news evidence, and derived signal event layers must be present, non-mock, have rows, and disclose source URLs.
...
- Artifact contract validation also rejects raw/scoring drift for duplicated optional provider payloads before workspace snapshot building.
...
- Workspace snapshot building rejects raw/scoring drift for duplicated optional payloads, including `announcements`, `announcement_evidence`, `valuation_snapshots`, `financial_metrics`, and `news_evidence`, before future web loading.
...
- Reports render optional `Announcements` and `Announcement Evidence` sections when CNINFO announcement metadata is present, including stock, title, category, date, narrative, confidence, generated summary, provider, source URL, and the PDF-not-parsed limitation.
...
- Reports render an optional `Market Quotes` section when `market_quotes` is present, including stock, latest price, change percent, change amount, previous close, volume, provider, and source URL.
