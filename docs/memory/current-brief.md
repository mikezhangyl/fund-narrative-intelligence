...
The Narrative Service promotion transaction boundary is now documented but not enabled: reserved command surface `POST /api/v1/narratives/promotion/commit`, all-or-none writes to trusted registry, trusted stock mapping, trusted evidence pack, and `narrative-promotion-decisions-v1` ledger, with `PD_*` immutable decision ids. Failed promotion commands must write no records; raw intake, review actions, and preflight remain non-promotional.
