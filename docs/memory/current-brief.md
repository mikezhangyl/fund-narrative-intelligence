...
On 2026-06-04, narrative source expansion changed to open-source-first for the current phase. Paid provider issues are Later/backlog; free/public/official sources and low-risk crawler pilots are the current path. Gateway now shares the FNI Linear project and owns all M20 external acquisition stories marked `[GATEWAY]`, including crawl governance runtime, SEC EDGAR, China official disclosures, policy/regulator/industry official sources, open news/RSS context, and controlled community/social heat. FNI owns `[FNI]` M20 consumer stories for source-quality coverage and fresh narrative digest. Canonical strategy: `docs/product/open-source-first-narrative-data-strategy-2026-06-04.html`.

On 2026-06-05, after stock-data-gateway merged PR #1, FNI switched its narrative source gateway consumer to the unified Gateway route `GET /api/v1/market-data/narrative/source-events`. `src/market_data/providers/narrative_source_gateway.py` now calls only that provider-neutral route for `official_filings`, `official_disclosures`, `official_sources`, `news_context`, `open_news_index`, `industry_media`, and `social_heat`. The legacy per-kind POST endpoints remain in `config/market_data_gateway_contract.yaml` only as compatibility declarations; FNI default probing should use `scripts/run_narrative_source_gateway_probe.py` against the unified route. Live local probe on 2026-06-05 returned 7 source kinds, 0 failed kinds, 6 rows total, with structured Gateway degradation for `open_news_index` timeout and disabled `social_heat`.

FNI M20 consumer artifacts now cover the downstream open-source source-event lane: source-quality dashboard rows one M20 Gateway source kind per row; `scripts/run_fresh_narrative_digest.py` preserves Gateway missing/degraded input diagnostics; `scripts/run_narrative_candidate_inbox.py` groups candidate narratives without automatic trust promotion; and `scripts/run_narrative_source_coverage_gap_report.py` produces the PM/Architect Gateway backlog gap report without auto-creating Linear issues.
