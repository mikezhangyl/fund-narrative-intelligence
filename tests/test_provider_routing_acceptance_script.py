...
import pytest
from scripts import validate_provider_routing_acceptance
from src.providers import routing as provider_routing


def test_provider_routing_acceptance_script_passes_with_mocked_cli(
    tmp_path,
    monkeypatch,
    capsys,
):
    ...
    def fake_main(args: list[str]) -> int:
        ...
        if args[:2] == ["--fund-code", "161725"]:
            _write_provider_routing_outputs(tmp_path)
        return 0

    monkeypatch.setattr(validate_provider_routing_acceptance.pipeline_main, "main", fake_main)

    exit_code = validate_provider_routing_acceptance.main(
        ["--output-dir", str(tmp_path)]
    )

    captured = capsys.readouterr()
    assert exit_code == 0
    assert "Provider routing acceptance passed:" in captured.out
    assert calls == [
        [
            "--fund-code",
            "161725",
            ...
            str(validate_provider_routing_acceptance.DEFAULT_ROUTING_CONFIG_PATH),
            "--output-dir",
            str(tmp_path),
        ],
...
def test_provider_routing_acceptance_rejects_missing_fallback_event(tmp_path):
    _write_provider_routing_outputs(tmp_path, omit_layer_fallback={"financial_metrics"})

    with pytest.raises(validate_provider_routing_acceptance.AcceptanceError) as exc:
        validate_provider_routing_acceptance.validate_acceptance_outputs(tmp_path)

    assert "missing required provider fallback event for financial_metrics" in str(
        exc.value
    )


def test_provider_routing_acceptance_can_validate_primary_success_mode(tmp_path):
    _write_provider_routing_outputs(
        tmp_path,
        market_provider="akshare-market-quote",
        valuation_provider="tushare-valuation",
        financial_provider="tushare-financial-metrics",
        layer_fallbacks={},
    ...
    validate_provider_routing_acceptance.validate_acceptance_outputs(
        ...
        forbid_provider_unavailable=("tushare-valuation", "tushare-financial-metrics"),
        expected_final_providers={
            "market_quotes": {"akshare-market-quote"},
...
def test_provider_routing_acceptance_rejects_forbidden_provider_unavailable_event(tmp_path):
    _write_provider_routing_outputs(
        tmp_path,
        market_provider="akshare-market-quote",
        valuation_provider="tushare-valuation",
    ...
    assert "unexpected provider_unavailable event for tushare-valuation" in str(exc.value)


def test_provider_routing_acceptance_forces_primary_providers_unavailable():
    original_market_provider = provider_routing.AkshareMarketDataProvider
    original_valuation_provider = provider_routing.TushareValuationProvider
    original_financial_provider = provider_routing.TushareFinancialMetricsProvider

    with validate_provider_routing_acceptance._forced_primary_unavailable_context():
        market_provider = provider_routing.AkshareMarketDataProvider()
        valuation_provider = provider_routing.TushareValuationProvider()
        financial_provider = provider_routing.TushareFinancialMetricsProvider()

        assert market_provider.get_stock_quotes(["600519"])["data_quality"] == "unavailable"
        assert valuation_provider.token == ""
        assert financial_provider.token == ""

    assert provider_routing.AkshareMarketDataProvider is original_market_provider
    assert provider_routing.TushareValuationProvider is original_valuation_provider
    assert (
        provider_routing.TushareFinancialMetricsProvider
        is original_financial_provider
    )


def _write_provider_routing_outputs(
    output_dir: Path,
